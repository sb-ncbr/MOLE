...
namespace WebChemistry.Util.WPF
{
    /// <summary>
    ...
    public class Trackball
    {
        public struct Params
        {
            ...
            public readonly Vector3D TranslationOffset;

            public Params(Vector3D axis, double angle, double scale, Vector3D offset)
            {
                RotationAxis = axis;
                RotationAngle = angle;
                Scale = scale;
        ...
        Subject<Params> paramsStream = new Subject<Params>();

        void UpdateParams(Params prms)
        {
            //var prms = new Params(
            //    _rotation.Axis,
            //    _rotation.Angle,
            //    _scale.ScaleX,
            //    new Vector3D(_translate.OffsetX, _translate.OffsetY, _translate.OffsetZ));

            Messenger.Default.Send<Params>(prms, "toVM");
        }
        
        public void SetParams(Params prms)
        {
            this.parameters = prms;

            if (_scale.ScaleX != prms.Scale)
            {
                _scale.ScaleX = prms.Scale;
                _scale.ScaleY = prms.Scale;
                _scale.ScaleZ = prms.Scale;
            }
            _rotation.Axis = prms.RotationAxis;
            _rotation.Angle = prms.RotationAngle;
            _translate.OffsetX = prms.TranslationOffset.X;
            _translate.OffsetY = prms.TranslationOffset.Y;
            _translate.OffsetZ = prms.TranslationOffset.Z;
        }

        public Trackball(double rotationFactor = 4.0, double zoomFacfor = 1.0)
        {
            paramsStream.Throttle(TimeSpan.FromMilliseconds(10), new DispatcherScheduler(Dispatcher.CurrentDispatcher)).Subscribe(p => UpdateParams(p));
        ...
        public bool IsSuppressed 
        {
            ...
            {
                isSuppressed = value;
                if (isSuppressed) Mouse.Capture(EventSource, CaptureMode.None);
            }
        }
        ...
        private void OnMouseMove(object sender, MouseEventArgs e)
        {
            ...
            if (update)
            {
                paramsStream.OnNext(new Params(
                    _rotation.Axis,
                    _rotation.Angle,
                    _scale.ScaleX,
                    new Vector3D(_translate.OffsetX, _translate.OffsetY, _translate.OffsetZ)));
        ...
        private void Look(Point currentPosition)
        {
            ...
            if (Math.Abs(axis.X + axis.Y + axis.Z) < 0.00001)
            {
                var diff = _previousPosition3D - currentPosition3D;
                if (Math.Abs(diff.X) > 0.00001) axis = new Vector3D(0, 1, 1);
                if (Math.Abs(diff.Y) > 0.00001) axis = new Vector3D(1, 0, 1);
                if (Math.Abs(diff.Z) > 0.00001) axis = new Vector3D(1, 1, 0);
            }
